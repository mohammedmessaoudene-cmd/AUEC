...
def verify_result(
    ...
    if (
        ...
        or result.get("auecVersion") != "0.1"
        or result.get("status") != "succeeded"
    ):
        raise AUECError("E_VERIFY", "result is not a successful AUEC result")
    if manifest is not None:
        if result.get("manifestId") != manifest.get("manifestId") or result.get(
            "manifestDigest"
    ...
    for index, receipt in enumerate(receipts, start=1):
        ...
        if (
            receipt.get("sequence") != index
            or receipt.get("previousReceiptDigest") != previous
        ):
        ...
        if receipt.get("manifestId") != result.get("manifestId") or receipt.get(
            "manifestDigest"
        ...
        previous = expected
    if (
        result.get("receiptCount") != len(receipts)
        or result.get("lastReceiptDigest") != previous
    ):
    ...
    for node_id, envelope in exports.items():
        if node_id not in by_node or by_node[node_id].get(
            "outputDigest"
