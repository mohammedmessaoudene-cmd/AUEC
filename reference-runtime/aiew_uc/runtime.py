...
from .canonical import canonical_json_bytes, canonical_json_text, digest_json
from .errors import AUECError
from .model import (
    CLASS_RANK,
    ValidatedManifest,
    classification_max,
    default_host_policy as _default_policy,
    validate_manifest,
)
...
def _replace_literal(text: str, needles: list[str], replacement: str) -> str:
    ordered = sorted(
        set(needles), key=lambda item: (-len(item), item.encode("utf-16-be"))
    )
    ...
    while index < len(text):
        match = next(
            (needle for needle in ordered if text.startswith(needle, index)), None
        )
        if match is None:
...
def _execute_op(op: str, inputs: dict[str, Any], params: dict[str, Any]) -> Any:
    ...
    if op == "text.search_literal":
        ...
        if len(needles) > 256 or any(
            not isinstance(item, str) or item == "" for item in needles
        ):
    ...
    if op == "text.redact_literal":
        if (
            set(inputs) != {"text"}
            or set(params) - {"needles", "replacement"}
            or "needles" not in params
        ):
    ...
    if op == "json.project":
        ...
        if (
            len(pointers) > 256
            or any(not isinstance(item, str) for item in pointers)
            or len(pointers) != len(set(pointers))
        ):
            raise AUECError("E_PARAMS", "pointers must be unique strings")
        return {
            pointer: deepcopy(_json_pointer(inputs["value"], pointer))
            for pointer in pointers
        }
    if op == "list.deduplicate":
        if set(inputs) != {"items"} or params:
            raise AUECError(
                "E_SIGNATURE", "list.deduplicate expects items and no params"
            )
        items = _require(inputs["items"], list, "items")
        seen: set[str] = set()
        result: list[Any] = []
    ...
    if op == "math.sum_safeint":
        if set(inputs) != {"values"} or params:
            raise AUECError(
                "E_SIGNATURE", "math.sum_safeint expects values and no params"
            )
        ...
        for value in values:
            ...
            if total < -9_007_199_254_740_991 or total > 9_007_199_254_740_991:
                raise AUECError(
                    "E_INTEGER_RANGE",
                    "sum exceeds the interoperable safe-integer range",
                )
        return total
...
def _choose_placement(node: dict[str, Any], policy: dict[str, Any]) -> str:
    permitted = [
        item
        for item in node["placement"]["allowed"]
        if item in policy["allowedPlacements"]
    ]
    if not permitted:
...
class UniversalRuntime:
    ...
    def __init__(self, host_policy: dict[str, Any] | None = None) -> None:
        self.host_policy = (
            default_host_policy() if host_policy is None else deepcopy(host_policy)
        )
    ...
    def execute(self, manifest: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            manifest_id = (
                ...
                and isinstance(manifest.get("manifestId"), str)
                else None
            )
            result: dict[str, Any] = {
                "auecVersion": "0.1",
    ...
    def _execute_validated(
        self, validated: ValidatedManifest, started_ns: int
    ) -> dict[str, Any]:
        ...
        for sequence, node_id in enumerate(validated.order, start=1):
            ...
            for name, expr in node["inputs"].items():
                value, classification = _resolve_expr(
                    expr, manifest["resources"], outputs
                )
                resolved[name] = value
            ...
            if CLASS_RANK[declared] < CLASS_RANK[inherited]:
                raise AUECError(
                    "E_CLASSIFICATION_DOWNGRADE",
                    "output classification is below input classification",
                )
        ...
        for node_id in validated.order:
            ...
            if CLASS_RANK[output["classification"]] > max_export_rank:
                raise AUECError(
                    "E_EGRESS_CLASSIFICATION",
                    "output classification exceeds host egress policy",
                )
            if (
                output["epistemic"] in {"claim", "hypothesis"}
                and not policy["allowClaimExport"]
            ):
                raise AUECError(
                    "E_EPISTEMIC_EGRESS", "claim or hypothesis export is forbidden"
                )
