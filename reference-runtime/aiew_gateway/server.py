...
def _protocol_json_bytes(value: Any) -> bytes:
    ...
    try:
        ...
        raise AUECError("E_PROTOCOL_JSON_ENCODE", "wire value is not valid JSON") from exc
...
class GatewayHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(self, server_address: tuple[str, int], state: GatewayState | None = None):
...
class GatewayRequestHandler(BaseHTTPRequestHandler):
    ...
    def _send_a2a_json(self, status: int, value: Any, *, headers: dict[str, str] | None = None) -> None:
    ...
    @staticmethod
    def _a2a_error_detail(reason: str, metadata: dict[str, str] | None = None) -> dict[str, Any]:
        detail: dict[str, Any] = {
    ...
    def _validate_a2a_service_parameters(
        ...
        *,
        ...
        if version != A2A_PROTOCOL_VERSION:
            if jsonrpc:
                self._send_json(400, self._rpc_error(
                    request_id,
                    -32009,
                ...
                self._a2a_rest_error(
                    ...
                    metadata={"requestedVersion": version, "supportedVersions": A2A_PROTOCOL_VERSION},
                )
            return False
    ...
    def _discard_optional_body(self) -> None:
        ...
        if transfer_encoding and transfer_encoding.lower().strip() not in {"", "identity"}:
            self.close_connection = True
            raise AUECError("E_HTTP_TRANSFER_ENCODING", "chunked request bodies are not supported")
    ...
    def _is_modern_mcp(self, method: str, params: dict[str, Any]) -> bool:
        ...
        return method == "server/discover" or has_modern_meta or self.headers.get("MCP-Protocol-Version") == MCP_MODERN_VERSION
    ...
    def _validate_modern_mcp_headers_and_meta(
        ...
        if not isinstance(meta, dict):
            return None, (400, self._rpc_error(request_id, -32602, "Invalid params: required _meta object is missing"))
        version = meta.get("io.modelcontextprotocol/protocolVersion")
        capabilities = meta.get("io.modelcontextprotocol/clientCapabilities")
        if not isinstance(version, str) or not version:
            return None, (400, self._rpc_error(request_id, -32602, "Invalid params: protocolVersion metadata is required"))
        if not isinstance(capabilities, dict):
            return None, (400, self._rpc_error(request_id, -32602, "Invalid params: clientCapabilities metadata is required"))
        ...
        if client_info is not None and (
            ...
            return None, (400, self._rpc_error(request_id, -32602, "Invalid params: clientInfo must contain name and version"))

        accept = self.headers.get("Accept", "")
        accepted = {part.split(";", 1)[0].strip().lower() for part in accept.split(",") if part.strip()}
        if not {"application/json", "text/event-stream"}.issubset(accepted):
            return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: Accept must list application/json and text/event-stream"))

        header_version = self.headers.get("MCP-Protocol-Version")
        header_method = self.headers.get("Mcp-Method")
        if header_version != version:
            return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: MCP-Protocol-Version does not match request metadata"))
        if header_method != method:
            return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: Mcp-Method does not match the JSON-RPC method"))
        ...
        if method in {"tools/call", "prompts/get", "resources/read"}:
            if not isinstance(required_name, str):
                return None, (400, self._rpc_error(request_id, -32602, "Invalid params: named method requires name or uri"))
            raw_header_name = self.headers.get("Mcp-Name")
            if raw_header_name is None:
                return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: required Mcp-Name is missing"))
            try:
                decoded_name = self._decode_mcp_header_value(raw_header_name)
            except ValueError as exc:
                return None, (400, self._rpc_error(request_id, -32020, f"Header mismatch: {exc}"))
            if decoded_name != required_name:
                return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: Mcp-Name does not match the request body"))

        if method == "tools/call" and isinstance(required_name, str):
            descriptor = next(
                (item for item in self.server.state.mcp_tools_list().get("tools", []) if item.get("name") == required_name),
                None,
            )
            schema = descriptor.get("inputSchema", {}) if isinstance(descriptor, dict) else {}
            properties = schema.get("properties", {}) if isinstance(schema, dict) else {}
            arguments = params.get("arguments", {})
            if not isinstance(arguments, dict):
                return None, (400, self._rpc_error(request_id, -32602, "Invalid params: arguments must be an object"))
            if isinstance(properties, dict):
                for param_name, definition in properties.items():
                    ...
                    if not isinstance(suffix, str) or not suffix:
                        return None, (400, self._rpc_error(request_id, -32602, "Invalid params: malformed x-mcp-header annotation"))
                    if param_name not in arguments:
                        continue
                    body_value = arguments.get(param_name)
                    if not isinstance(body_value, str):
                        return None, (400, self._rpc_error(request_id, -32602, "Invalid params: header-bound value must be a string"))
                    raw_header = self.headers.get(f"Mcp-Param-{suffix}")
                    if raw_header is None:
                        return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: required custom parameter header is missing"))
                    try:
                        decoded_header = self._decode_mcp_header_value(raw_header)
                    except ValueError as exc:
                        return None, (400, self._rpc_error(request_id, -32020, f"Header mismatch: {exc}"))
                    if decoded_header != body_value:
                        return None, (400, self._rpc_error(request_id, -32020, "Header mismatch: custom parameter header does not match request body"))

        if version not in MCP_SUPPORTED_VERSIONS or version != MCP_MODERN_VERSION:
            return None, (400, self._rpc_error(
                request_id,
                -32022,
    ...
    def _a2a_error_mapping(code: str) -> tuple[int, int, str, str]:
        mapping = {
            "E_RPC_REQUEST": (-32600, 400, "INVALID_REQUEST", "Invalid JSON-RPC request"),
            "E_RPC_PARAMS": (-32602, 400, "INVALID_ARGUMENT", "Invalid JSON-RPC parameters"),
            "E_A2A_INVALID_PARAMS": (-32602, 400, "INVALID_ARGUMENT", "Invalid A2A parameters"),
            "E_A2A_IDEMPOTENCY": (-32602, 400, "INVALID_ARGUMENT", "Idempotency key collision"),
            "E_A2A_TASK_NOT_FOUND": (-32001, 404, "TASK_NOT_FOUND", "Task not found"),
            "E_A2A_TASK_NOT_CANCELABLE": (-32002, 409, "TASK_NOT_CANCELABLE", "Task is not cancelable"),
            "E_A2A_PUSH_NOT_SUPPORTED": (-32003, 400, "PUSH_NOTIFICATION_NOT_SUPPORTED", "Push notifications are not supported"),
            "E_A2A_UNSUPPORTED_OPERATION": (-32004, 400, "UNSUPPORTED_OPERATION", "Operation is not supported"),
            "E_A2A_CONTENT_TYPE": (-32005, 415, "CONTENT_TYPE_NOT_SUPPORTED", "Content type is not supported"),
            "E_A2A_INVALID_AGENT_RESPONSE": (-32006, 502, "INVALID_AGENT_RESPONSE", "Invalid agent response"),
            "E_A2A_EXTENDED_NOT_CONFIGURED": (-32007, 400, "EXTENDED_AGENT_CARD_NOT_CONFIGURED", "Extended agent card is not configured"),
            "E_A2A_EXTENSION_REQUIRED": (-32008, 400, "EXTENSION_SUPPORT_REQUIRED", "Required extension support is missing"),
            "E_A2A_VERSION": (-32009, 400, "VERSION_NOT_SUPPORTED", "A2A protocol version is not supported"),
        }
        return mapping.get(code, (-32602, 400, "INVALID_ARGUMENT", "Invalid A2A parameters"))
    ...
    def _send_a2a_rpc_exception(self, request_id: Any, exc: AUECError) -> None:
        ...
        data = [self._a2a_error_detail(reason, {"aiewCode": exc.info.code})] if rpc_code <= -32001 and rpc_code >= -32099 else None
        self._send_json(
            200,
    ...
    def _send_a2a_rest_exception(self, exc: AUECError) -> None:
        ...
        self._a2a_rest_error(http_status, reason, message, metadata={"aiewCode": exc.info.code})
    ...
    def _payload_contains_auec_manifest(payload: Any) -> bool:
        ...
        for part in parts:
            ...
            if isinstance(data, dict) and (
                isinstance(data.get("manifest"), dict) or data.get("auecVersion") == "0.1"
            ):
                return True
        return False

    def _require_aiew_extension_if_needed(self, payload: Any, *, request_id: Any | None, jsonrpc: bool) -> bool:
        ...
        if jsonrpc:
            self._send_json(
                ...
                self._rpc_error(
                    ...
                    -32008,
                    "Required A2A extension was not activated",
                    [self._a2a_error_detail("EXTENSION_SUPPORT_REQUIRED", {"requiredExtension": A2A_EXTENSION_URI})],
                ),
                headers={"A2A-Version": A2A_PROTOCOL_VERSION},
            )
        else:
    ...
    def _mcp_elicitation_params(self, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool_name == "test_elicitation_sep1330_enums":
            return {
                ...
                "requestedSchema": {
                    ...
                    "required": ["untitledSingle", "titledSingle", "legacyEnum", "untitledMulti", "titledMulti"],
                },
            }
        raise AUECError("E_MCP_TOOL_NOT_FOUND", "unknown elicitation fixture")

    def _handle_modern_mcp_progress_tool(self, request_id: Any, params: dict[str, Any]) -> bool:
    ...
    def _handle_modern_mcp_subscription(self, params: dict[str, Any]) -> None:
        notifications = params.get("notifications", {}) if isinstance(params, dict) else {}
        subscription_id, channel = self.server.state.open_mcp_subscription(notifications)
        self._begin_sse(headers={"MCP-Protocol-Version": MCP_MODERN_VERSION})
        try:
            self._write_sse_json({
                "jsonrpc": "2.0",
    ...
    def _handle_legacy_mcp_streaming_tool(self, request_id: Any, params: dict[str, Any]) -> bool:
        name = params.get("name") if isinstance(params, dict) else None
        if name not in {
        ...
        if not state.has_mcp_session(session_id):
            self._send_json(200, self._rpc_error(request_id, -32000, "MCP session required"))
            return True

        self._begin_sse(headers={"Mcp-Session-Id": str(session_id), "MCP-Protocol-Version": MCP_LEGACY_VERSION})
        try:
            if name == "test_tool_with_logging":
                for message in ("Tool execution started", "Tool processing data", "Tool execution completed"):
                    self._write_sse_json({
                        ...
                        "params": {"level": "info", "logger": "auec-gateway", "data": message},
                    })
                    time.sleep(0.055)
                ...
                if token is not None:
                    for progress in (0, 50, 100):
                        self._write_sse_json({
                            "jsonrpc": "2.0",
                            "method": "notifications/progress",
                            "params": {"progressToken": token, "progress": progress, "total": 100},
                        })
                        time.sleep(0.055)
                ...
                if name == "test_sampling":
                    ...
                    client_params = {
                        "messages": [{
                            "role": "user",
                            "content": {"type": "text", "text": arguments.get("prompt", "Test prompt for sampling")},
                        }],
                        "maxTokens": 100,
                    }
                else:
                ...
                client_request, event = state.create_mcp_client_request(str(session_id), client_method, client_params)
                ...
                if name == "test_sampling":
                    content = client_result.get("content", {}) if isinstance(client_result, dict) else {}
                    sample_text = content.get("text", "client sampling completed") if isinstance(content, dict) else "client sampling completed"
                    result = {"content": [{"type": "text", "text": f"LLM response: {sample_text}"}]}
                else:
                    action = client_result.get("action", "accept") if isinstance(client_result, dict) else "accept"
                    result = {"content": [{"type": "text", "text": f"Elicitation completed: action={action}"}]}
            self._write_sse_json(self._rpc_result(request_id, result))
        except AUECError as exc:
            self._write_sse_json(self._rpc_error(request_id, -32000, "AIEW gateway rejected request", {"aiewCode": exc.info.code}))
        except (BrokenPipeError, ConnectionResetError):
            return True
        except Exception:
            try:
                self._write_sse_json(self._rpc_error(request_id, -32603, "Internal error"))
            except (BrokenPipeError, ConnectionResetError):
                pass
        return True
    ...
    def do_GET(self) -> None:
        ...
        path = parsed.path
        if path == "/healthz":
            self._send_json(200, {"status": "ok", "name": "auec-gateway", "version": AIEW_GATEWAY_VERSION})
            return
        if path == "/source":
        ...
        if path == "/.well-known/agent-card.json":
            ...
            self._send_bytes(
                200,
                body,
                headers={"ETag": etag, "Last-Modified": last_modified, "A2A-Version": A2A_PROTOCOL_VERSION},
                cache_control="public, max-age=300",
            )
            return
        if path == "/extendedAgentCard":
            if not self._validate_a2a_service_parameters(request_id=None, jsonrpc=False, require_extension=False):
                return
            self._send_a2a_json(200, self.server.state.extended_agent_card(self.base_url))
            return
        if "/pushNotificationConfigs" in path:
            if not self._validate_a2a_service_parameters(request_id=None, jsonrpc=False, require_extension=False):
                return
            self._a2a_rest_error(400, "PUSH_NOTIFICATION_NOT_SUPPORTED", "Push notifications are not supported")
            return
        if path.startswith("/tasks/") and path.endswith(":subscribe"):
            if not self._validate_a2a_service_parameters(request_id=None, jsonrpc=False, require_extension=False):
                return
            self._a2a_rest_error(400, "UNSUPPORTED_OPERATION", "Streaming is not supported")
            return
        if path.startswith("/tasks/"):
            ...
            if "historyLength" in query:
                try:
                    ...
                    self._a2a_rest_error(400, "INVALID_ARGUMENT", "Invalid historyLength")
                    return
            try:
        ...
        if path == "/tasks":
            ...
            if "includeArtifacts" in query:
                ...
                if raw not in {"true", "false"}:
                    self._a2a_rest_error(400, "INVALID_ARGUMENT", "Invalid includeArtifacts")
                    return
            ...
            return
        if path == "/mcp":
            self._send_json(405, {"error": "MCP standalone SSE GET transport is not enabled"}, headers={"Allow": "POST, DELETE"})
            return
    ...
    def do_DELETE(self) -> None:
        ...
        if path == "/mcp":
            if self.headers.get("MCP-Protocol-Version") == MCP_MODERN_VERSION:
                self._send_json(405, {"error": "method_not_allowed"}, headers={"Allow": "POST"})
            else:
                session_id = self.headers.get("Mcp-Session-Id")
                if session_id is not None and not self.server.state.unregister_mcp_session(session_id):
                    self._send_json(404, self._rpc_error(None, -32001, "Unknown MCP session"))
                else:
                    self._send_json(200, {})
            return
        if "/pushNotificationConfigs/" in path:
    ...
    def do_POST(self) -> None:
        ...
        a2a_rest_paths = (
            ...
            or (path.startswith("/tasks/") and (path.endswith(":cancel") or path.endswith(":subscribe")))
        ...
        if a2a_rest_paths:
            ...
            if content_type not in {"application/json", "application/a2a+json"}:
                ...
                self.close_connection = True
                self._a2a_rest_error(415, "CONTENT_TYPE_NOT_SUPPORTED", "Content type is not supported")
                return
        if path in {"/", "/a2a"}:
            content_type = self.headers.get("Content-Type", "").split(";", 1)[0].strip().lower()
            if content_type not in {"application/json", "application/a2a+json"}:
                ...
                self._send_json(
                    ...
                    headers={"A2A-Version": A2A_PROTOCOL_VERSION, "Connection": "close"},
                )
                return
        if path == "/mcp" and not self._origin_allowed():
            self.close_connection = True
            self._send_json(403, self._rpc_error(None, -32023, "Forbidden Origin"), headers={"Connection": "close"})
            return
        try:
            payload = self._read_json()
        except AUECError as exc:
            if path == "/mcp":
                self._send_json(400, self._rpc_error(None, -32700, "Parse error", {"aiewCode": exc.info.code}))
            elif path in {"/", "/a2a"}:
                self._send_json(
                    400,
        ...
        if path == "/message:send":
            if not self._validate_a2a_service_parameters(request_id=None, jsonrpc=False, require_extension=False):
                return
            if not self._require_aiew_extension_if_needed(payload, request_id=None, jsonrpc=False):
                return
            try:
    ...
    def _validate_rpc_request(self, payload: Any) -> tuple[Any, str, dict[str, Any], bool]:
    ...
    def _handle_mcp(self, payload: Any) -> None:
        ...
        if (
            ...
            if state.resolve_mcp_client_response(session_id, payload):
                ...
                self._send_json(400, self._rpc_error(payload.get("id"), -32600, "Unknown client response"))
            return

        try:
            request_id, method, params, is_notification = self._validate_rpc_request(payload)
            modern = self._is_modern_mcp(method, params)

            if modern:
                _meta, rejection = self._validate_modern_mcp_headers_and_meta(request_id, method, params)
                ...
                if method == "server/discover":
                    ...
                    result = state.modern_result(method, state.mcp_resource_templates_list())
                elif method == "resources/read":
                    result = state.modern_result(method, state.mcp_resource_read(params))
                    ...
                    result = state.modern_result(method, state.mcp_completion_complete(params))
                else:
                    self._send_json(404, self._rpc_error(request_id, -32601, "Method not found"))
                    return
                if is_notification:
                    self._send_bytes(202, b"", content_type="application/json")
                else:
                    self._send_json(200, self._rpc_result(request_id, result), headers={"MCP-Protocol-Version": MCP_MODERN_VERSION})
                return
            ...
            if method == "initialize":
                ...
                headers = {"Mcp-Session-Id": session_id, "MCP-Protocol-Version": MCP_LEGACY_VERSION}
                ...
                result, headers = {"completion": {"values": [], "total": 0, "hasMore": False}}, {}
            else:
                self._send_json(200, self._rpc_error(request_id, -32601, "Method not found"))
                return
            if is_notification:
                self._send_bytes(202, b"", content_type="application/json", headers=headers)
            else:
                self._send_json(200, self._rpc_result(request_id, result), headers=headers)
        except AUECError as exc:
            ...
            if modern and exc.info.code == "E_MCP_MISSING_CAPABILITY":
                ...
                self._send_json(400, self._rpc_error(
                    request_id, -32021, "Missing required client capability", {"requiredCapabilities": {capability: {}}}
                ))
                return
            if modern:
                code = -32602 if exc.info.code in {
                    "E_RPC_PARAMS", "E_MCP", "E_MCP_STATE", "E_MCP_TOOL_NOT_FOUND", "E_MCP_RESOURCE_NOT_FOUND", "E_MCP_PROMPT_NOT_FOUND"
                } else -32000
                data: dict[str, Any] = {"aiewCode": exc.info.code}
                if exc.info.code == "E_MCP_RESOURCE_NOT_FOUND" and isinstance(params, dict) and isinstance(params.get("uri"), str):
                    data["uri"] = params["uri"]
                self._send_json(400, self._rpc_error(request_id, code, "AIEW gateway rejected request", data))
            else:
                rpc_code = -32602 if exc.info.code in {"E_RPC_REQUEST", "E_RPC_PARAMS", "E_MCP"} else -32000
                self._send_json(200, self._rpc_error(request_id, rpc_code, "AIEW gateway rejected request", {"aiewCode": exc.info.code}))
        except Exception:
            request_id = payload.get("id") if isinstance(payload, dict) else None
    ...
    def _handle_a2a(self, payload: Any) -> None:
        request_id = payload.get("id") if isinstance(payload, dict) else None
        if not self._validate_a2a_service_parameters(request_id=request_id, jsonrpc=True, require_extension=False):
            return
        if not self._require_aiew_extension_if_needed(payload, request_id=request_id, jsonrpc=True):
            return
        try:
            request_id, method, params, is_notification = self._validate_rpc_request(payload)
            if is_notification:
                self._send_bytes(202, b"", content_type="application/json", headers={"A2A-Version": A2A_PROTOCOL_VERSION})
                return
            state = self.server.state
            if method == "SendMessage":
                ...
                raise AUECError("E_A2A_UNSUPPORTED_OPERATION", "streaming not supported")
            elif method in {
                "CreateTaskPushNotificationConfig", "GetTaskPushNotificationConfig",
                "ListTaskPushNotificationConfigs", "DeleteTaskPushNotificationConfig",
            }:
                raise AUECError("E_A2A_PUSH_NOT_SUPPORTED", "push notifications not supported")
            else:
                self._send_json(200, self._rpc_error(request_id, -32601, "Method not found"), headers={"A2A-Version": A2A_PROTOCOL_VERSION})
                return
            self._send_json(200, self._rpc_result(request_id, result), headers={"A2A-Version": A2A_PROTOCOL_VERSION})
        except AUECError as exc:
...
def create_server(
    ...
    *,
    ...
    enforce = (not _host_is_loopback(host)) if require_exact_source is None else require_exact_source
    if enforce:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--require-exact-source-offer", action="store_true", help="fail startup unless the AGPL source URL, ref, and SHA-256 are exact")
    ...
    if args.source_offer:
        print(json.dumps(source_offer_payload(), ensure_ascii=False, sort_keys=True, separators=(",", ":")))
        return 0

    server = create_server(args.host, args.port, require_exact_source=(args.require_exact_source_offer or not _host_is_loopback(args.host)))
    address, port = server.server_address
    if args.ready_file:
        with open(args.ready_file, "w", encoding="utf-8", newline="\n") as handle:
            json.dump({"host": address, "port": port}, handle, sort_keys=True, separators=(",", ":"))
            handle.write("\n")
