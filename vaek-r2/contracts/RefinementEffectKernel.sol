...
try IServicePurchaseAdapter(purchaseAdapter).purchase(
executionId,
request.target,
request.subjectId,
request.asset,
evaluation.authorizedAmount,
request.contextHash
) returns (uint256 charged, bytes32 adapterOutcomeHash) {
...
bool humanRequired = risk >= RiskTier.HIGH
|| (mandate.humanGateAbove > 0 && authorizedAmount > mandate.humanGateAbove);
...
function _riskTier(EffectType effectType, uint128 amount, uint128 maxPerEffect)
internal
pure
returns (RiskTier)
{
...
if (bps > 7_500) return RiskTier.HIGH;
if (bps > 2_500) return RiskTier.MEDIUM;
return RiskTier.LOW;
}
if (bps > 9_000) return RiskTier.HIGH;
if (bps > 5_000) return RiskTier.MEDIUM;
return RiskTier.LOW;
}
